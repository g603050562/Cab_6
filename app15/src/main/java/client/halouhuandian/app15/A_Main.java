 ...
 * 软件首页
 */
public class A_Main extends Activity implements OnClickListener, MyApplication.IFResultAppLinstener, OpenLongLink.IFHttpOpenLongLinkLinstener, IFHttpBandLongLinkLinstener,
    ...
    private LinearLayout into_admin;
    private ImageView black_1, black_2;
    private ImageView up_bar_1, up_bar_2;
    private ImageView rent_bar_qcode, main_logo;
    ...
    private LinearLayout info_panel, cost, main_tel;
    ...
    public static Handler chargeOnHandler, chargeOffHandler,
            speakHandler, showProgressDialogHandler, openADCDHandler, closeADCDHandler, adminElongationHandler, adminShrinkHandler, writeUidHandler;
    ...
    public static Map<Integer, Boolean> replaceStatusMap = new ConcurrentHashMap<>();
...
//            code = "-1";
    ...
    IFHttpOutLineCheckOldBindListener ifHttpOutLineCheckOldBindListener = new IFHttpOutLineCheckOldBindListener() {
        ...
        public void onHttpOutLineCheckOldBindResult(String in_door, String code, String str, String data) {
            ...
            if (code.equals("-1")) {  //开启离线换电
                showDialogInfo("该电池未绑定，将被回收，如有问题请拨打客服客服！", "10", "1");
                //设置充电状态
                AN_IS_RUN = 0;
            } else {
    ...
    private Consumer remoteOpenDoorConsumer, remoteCloseDoorConsumer;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        activity = this;
        LogUtil.I("Activity：A_Main onCreate");
        setContentView(R.layout.activity_main_1080p_new);
...
//        rootCommand.execRootCmd(packegName_10);
        ...
        progressDialog_4.show("电柜正在初始化，请稍候！", 30, 0);
        ...
        final Handler onCreateHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                boolean is_frist = cabInfoSp.getFristOpen();
                ...
                if (is_frist == false) {
                    ...
                    LogUtil.I("网络：   电话-" + cabInfoSp.getTelNumber());
        ...
        if (false) {
            new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
                    //                    Intent intent1 = new Intent(A_Main.this, A_UpDataBattery.class);
                    //                    intent1.putExtra("door", "1");
                    ...
                    //                    intent1.putExtra("path", targetPath);
                    //                    intent1.putExtra("manu", "D");
                    //                    intent1.putExtra("cabid", "你好啊。。。");
                    //                    intent1.putExtra("tel", "18699993333");

//        Intent intent1 = new Intent(A_Main.this, DCDC_UpgradeActivity.class);
//        intent1.putExtra("door", "1");
//        String fileName = "TDK1000V75Z_HV01SV13_20200116.bin";
//        String targetPath = "/sdcard/Download/pdu/" + fileName;
//        intent1.putExtra("path", targetPath);
//        intent1.putExtra("cabid", "你好啊。。。");
//        intent1.putExtra("tel", "18699993333");

                    //                Intent intent1 = new Intent(A_Main.this, ACDC_UpgradeActivity.class);
                    //                intent1.putExtra("door", 0x52+"");
                    //                String fileName = "TC096K3000M1S_HV01SV02_20191225.bin";
                    //                String targetPath = "/sdcard/Download/pdu/" + fileName;
                    //                intent1.putExtra("path", targetPath);
                    //                intent1.putExtra("cabid", "你好啊。。。");
                    //                intent1.putExtra("tel", "18699993333");
...
//                    hun_str = "0";
        ...
        if (false) {
            new Handler().postDelayed(new Runnable() {
                @Override
                public void run() {
                    Intent intent1 = new Intent(A_Main.this, EvnBordUpdateActivity.class);
                    String fileName = "Sensor.bin";
                    String targetPath = "/sdcard/Download/" + fileName;
                    intent1.putExtra("path", targetPath);
                    intent1.putExtra("cabid", "你好啊。。。");
                    intent1.putExtra("tel", "18699993333");
                    startActivity(intent1);
...
//        }).start();
    ...
    private void findView() {
        into_admin = (LinearLayout) findViewById(R.id.into_admin);
        into_admin.setOnClickListener(this);
        ...
        info_panel = (LinearLayout) this.findViewById(R.id.info_panel);
        ...
        main_logo = (ImageView) this.findViewById(R.id.main_logo);
        main_logo.setOnClickListener(this);
        main_tel = (LinearLayout) this.findViewById(R.id.main_tel);
        main_tel.setOnClickListener(this);
        download_qcode = (ImageView) this.findViewById(R.id.download_qcode);
        download_qcode.setImageBitmap(Unit.generateBitmap("http://app.halouhuandian.com/App/jump", 400, 400));
        ...
        tel_text = this.findViewById(R.id.tel_text);
        ...
        tvEvnVersion = findViewById(R.id.tv_evnVersion);
...
//                    LogUtil.I("CAN - 返回 - BAR ：  " + door + " - 丢帧");
    ...
    private class DataHandleBARThread extends Thread {
        ...
        public void run() {
            ...
            if (data.length == 126 && AN_IS_RUN == 0 && upgrading.isEmpty()) {
                ...
                boolean isBdChange = !BIDS[i_address - 1].equals(bd);
                BIDS[i_address - 1] = bd;
                //电池UID
                UIDS[i_address - 1] = map.get("UID");

                final int doorIndex = i_address - 1;

                if (i_address > 0 && isInitFinish) {
                    if (DOORS[i_address - 1] == 0 && BIDS[doorIndex].equals("0000000000000000")) {
                        replaceStatusMap.remove(doorIndex);
                        return;
                    }
                    if (isCheckedBattery || isBdChange) {
                        if (replaceStatusMap.containsKey(doorIndex)) {
                            return;
                        } else {
                            if (SMALLS[doorIndex] == 1) {
                                return;
                            }
                            replaceStatusMap.put(doorIndex, true);
                        }

                        // TODO: 2020/7/19 出发换电操作
                        try {
                            syncControl(new Consumer() {
                                @Override
                                public void accept(Object o) {
                                    if (forbiddenSp.getTargetForbidden(doorIndex) != 1) {
                                        showDialogInfo("仓门被禁用，请更换其他仓门尝试！", "5", "1");
                                        return;
                                    } else {
                                        int lockTimes = 3;
                                        while (lockTimes-- > 0) {
                                            if (DOORS[i_address - 1] != 1) {
                                                SystemClock.sleep(1000);
                                            } else {
                                                break;
                                            }
                                        ...
                                        if (DOORS[i_address - 1] != 1) {
                                            showDialogInfo("仓门异常，请更换其他仓门尝试！", "10", "1");
                                            forbiddenSp.setTargetForbidden(doorIndex, -1);
                                            final int emptyDoor = obtainEmptyDoor();
                                            LogUtil.I("emptyDoor：" + emptyDoor);
                                            if (emptyDoor != -2) {
                                                if (!isValidDoor(emptyDoor)) {
                                    ...
                                    showDialogInfo("正在检测电池，请稍后！", "10", "1");
                                        ...
                                        if (dcdcStatus.equals("7")) {
                                            ...
                                            while (System.currentTimeMillis() - startTime < 40 * 1000) {
                                                ...
                                                if (!BIDS[doorIndex].equals("FFFFFFFFFFFFFFFF")
                                                        && !BIDS[doorIndex].equals("0000000000000000")) {
                                                    break;
                                                } else if (!TextUtils.isEmpty(dcdcStatus) && dcdcStatus.equals("8")) {
                                                    break;
                                                }
                                    ...
                                    if (!BIDS[doorIndex].equals("FFFFFFFFFFFFFFFF")
                                        ...
                                        if (DOORS[i_address - 1] == 1) {
                                            ...
                                            pull("换电触动微动收回仓门", i_address + "");
                                            LogUtil.I("正在关闭" + i_address + "号仓门，请注意安全！");
                                            SystemClock.sleep(6000);

                                            if (SMALLS[i_address - 1] == 1 && !BIDS[doorIndex].equals("FFFFFFFFFFFFFFFF")
                                                    && !BIDS[doorIndex].equals("0000000000000000")) {
                                                showDialogInfo("正在校验电池信息，请稍候！", "20", "1");
                                                new ExchangeBarThread(i_address).run();
                                            } else {
                                                LogUtil.I("侧微动：" + SMALLS[i_address - 1]);
    ...
    private void handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                try {
                    if (PERCENtAGES[i] == -1) {
                        ...
                        int c = PERCENtAGES[i];
                        bar_r_texts[i].setText(PERCENtAGES[i] + "%");
        ...
        changeQcodeHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                for (int i = 0; i < BIDS.length; i++) {
                    if (!BIDS[i].equals("0000000000000000") && !BIDS[i].equals("FFFFFFFFFFFFFFFF") && !BIDS[i].equals("FFFFFFFFFFFFFFFF0")) {
                        rent_bar_qcode.setImageBitmap(Unit.generateBitmap(cabinetID + "/" + BIDS[i] + "/" + (i + 1), 400, 400));
                        a = 1;
                        break;
                    }
                }
                if (a == 0) {
                    rent_bar_qcode.setImageBitmap(Unit.generateBitmap(cabinetID + "/" + "FFFFFFFFFFFFFFFF" + "/" + "0", 400, 400));
                }
            }
        };
        ...
        //:todo 写入电池UID（电池校验码）   类型：handler    参数：uid - 电池UID   door - 选择仓门
        writeUidHandler = new Handler() {
            final byte[] CHECK_CODE_BYTES = new byte[]{0x65, 0x00, 0x05, (byte) 0x98
                    , 0x08
                    , 0x00, 0x00, 0x00
            ...
            public void handleMessage(Message msg) {
                ...
                final String writeUid = msg.getData().getString("uid");
                final int door_id = msg.getData().getInt("door");
        ...
        sensorCollectionHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                tvEvnVersion.setText(evnSoftwareVersion);
            }
        };
...
//                new RootCommand().RootCommandStart("reboot");
        ...
        dcdcUpdradeHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                Intent intent = new Intent(activity, DcdcUpdateActivity.class);
                intent.putExtra("door", door);
                intent.putExtra("path", path);
                intent.putExtra("tel", cabInfoSp.getTelNumber());
                intent.putExtra("cabid", cabid_title);
        ...
        acdcUpdradeHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                Intent intent = new Intent(activity, AcdcUpdateActivity.class);
                intent.putExtra("door", door);
                intent.putExtra("path", path);
                intent.putExtra("tel", cabInfoSp.getTelNumber());
                intent.putExtra("cabid", cabid_title);
        ...
        setMoviesHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                LogUtil.I("movies：   反馈 - " + is_movies + "   SD卡数量 - " + PubFunction.getExtSDCardPathList().size());
        ...
        otherInfo_8_Handler = new Handler() {
            @Override
            public void handleMessage(Message msg) {
                super.handleMessage(msg);
        ...
        public void run() {
            ...
            if (NetUtil.isNetworkConnected(getApplicationContext())) {
                ...
                if (BatteryUtil.is8A(uid)) {
                    ...
                    HttpOutLineCheckUserBalance httpOutLineCheckUserBalance = new HttpOutLineCheckUserBalance(cabinetID, UIDS[inputBar - 1], BIDS[inputBar - 1], PERCENtAGES[inputBar - 1] + "", (inputBar) + "", dbm, ifHttpOutLineCheckUserBalanceListener);
    ...
    public void outLineExchangeUID(final int door) {
        ...
        if (doorIndex >= 0) {
            ...
            while (tryWriteCount > 0) {
                ...
                if (UIDS[out_door - 1].equals(inDoorBarUID)) {
                    result_out = 1;
                    break;
                }
                tryWriteCount--;
            ...
            if (result_out == 0) {
                ...
                if (exchangeFailCount == 0) {
                    showDialogInfo("电池校验失败，正在尝试再次换电，请稍后！", "7", "1");
                ...
                exchangeFailCount = 0;
                int thread_status = 300;
                while (thread_status > 0) {
                    SystemClock.sleep(100);
                    if (UIDS[in_door - 1].equals("AAAAAAAA")) {
                        thread_status = -100;
                    } else {
                        //下发清除插入电池ID
                        if (thread_status % 50 == 0) {
                            writeBatteryCheckCode("AAAAAAAA", in_door);
                        }
                        thread_status = thread_status - 1;
                    }
                }
                ...
                //UID计算电话号码
                String phoneInfo = BitUtil.bit32To10(uid);
                if (!TextUtils.isEmpty(phoneInfo)) {
                    if (phoneInfo.length() == 12) {
                        phoneInfo = phoneInfo.substring(7, 11);
     ...
     * @param door
     */
    private void writeBatteryCheckCode(String uid, int door) {
        Message message = writeUidHandler.obtainMessage();
        Bundle bundle_2 = new Bundle();
        bundle_2.putString("uid", uid);
        bundle_2.putInt("door", door);
        message.setData(bundle_2);
        writeUidHandler.sendMessage(message);
    }
     ...
     * @param door
     */
    private void pull(final String info, final String door) {
        Message message_2 = adminShrinkHandler.obtainMessage();
        Bundle bundle_2 = new Bundle();
        bundle_2.putString("info", info);
        bundle_2.putString("door", door);
    ...
    private Thread thread_send_can = new Thread() {
        ...
        public void run() {
            ...
            try {
                showDialogInfo("正在初始化仓门信息", "30", "0");
                sleep(1000 * 10);
                //开启启动 打开一个空的仓门
                boolean isHaveEmptyDoor = false;
                for (int i = 0; i < 9; i++) {
                    int is_stop = forbiddenSp.getTargetForbidden(i);
                    if (SMALLS[i] == 0) {
                        if (!isHaveEmptyDoor) {
                            if (is_stop == 1 && ("0000000000000000".equals(BIDS[i]) || "FFFFFFFFFFFFFFFF".equals(BIDS[i]))) {
                                isHaveEmptyDoor = true;
                                continue;
                            }
                        }

                        showDialogInfo("正在关闭" + (i + 1) + "仓门", "5", "0");
                        initPull("电柜开机关闭仓门", (i + 1) + "");
                        Thread.sleep(6000);
                    }
                }
                if (!isHaveEmptyDoor) {
                    final int noBarDoor = obtainEmptyDoor();
                    if (noBarDoor != -2) {
                        if (noBarDoor == -1) {
                            showDialogInfo("没有满足打开条件的仓门", "10", "0");
                        } else {
                            showDialogInfo("正在打开换电仓门", "5", "0");
                            push("info", noBarDoor + "");
                            SystemClock.sleep(6000);
                        }
                    }
                ...
                while (send_can_type_code == 0) {
                    ...
                    if (AN_IS_RUN == 0) {
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                t_9.setText(NetUtil.isNetworkConnected(getApplicationContext()) ? "在线" : "无网络");
                            }
                        });
...
//        MyApplication.serialAndCanPortUtils.onDestroy();
    ...
    private Handler QcodeHadnler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            download_qcode.setImageBitmap(Unit.generateBitmap(url, 400, 400));
            LogUtil.I("网络：   二维码 - " + url);
        }
    };
    ...
    public void onHttpReturnDataResult(final String orderString) {
        try {
            ...
            //长链接下发  打开关闭后台
                ...
                if (action.equals("1")) {
                    if (MyApplication.activitys.size() == 1) {
                        activity.startActivity(new Intent(activity, A_Admin.class));
                    }
                }
                if (action.equals("0")) {
                    if (MyApplication.activitys.size() == 2) {
                        MyApplication.activitys.get(1).finish();
                        MyApplication.activitys.remove(1);
                    }
                }
            }
            ...
            //长链接下发  更新柜子android软件
                ...
                showDialogInfo("准备更新主程序，请勿进行操作！", "10", 1 + "");
            ...
            //长链接下发  网络后台开门
                ...
                try {
                    ...
                    syncControl(new Consumer() {
                        @Override
                        public void accept(Object o) {
                            showDialogInfo("远程打开" + door + "号仓门", "10", "1");
            ...
            //长链接下发  给服务器上传电柜里面的电池参数
                ...
                for (int i = 0; i < DEFINE_BAR_COUNT; i++) {
                    ...
                    try {
                        ...
                        jsonObject_1.put("bty_rate", PERCENtAGES[i]);  //电池电量
                        //第二次上传参数
                        jsonObject_1.put("soh", "100");  //soh
                        jsonObject_1.put("soh_2", SOH_2[i]);  //soh
                        jsonObject_1.put("soc", realSocVs[i]);  //soc（跟电量一样）
                        //第三次上传参数
                        jsonObject_1.put("vdif", "0");  //电池里面各个串数电池 最高和最低的差值    压差
                        jsonObject_1.put("uses", LOOPS[i]);  //循环次数
                        //第四次上传参数
                        jsonObject_1.put("wendu", TEM_2[i] / 10); //电池温度
                        jsonObject_1.put("dianya", DIANYA[i]); //电池 电压
                        jsonObject_1.put("dianliu", DIANLIU[i]); //电池 电流
                        //第五次上传参数
                        ...
                        jsonObject_1.put("full_cap", FULL_CAP[i]); // 充满电池容量
                        jsonObject_1.put("left_cap", LEFT_CAP[i]); // 剩余电池容量

                        jsonObject_1.put("TEM_2", TEM_2[i] / 10);
                        ...
                        jsonObject_1.put("bsv", b_i); // 电池健康比 soh
                        jsonObject_1.put("bhv", a_i); // 电池健康比 soh

                        jsonObject_1.put("dcbsv", DCDC_SV[i]);
                        jsonObject_1.put("dcbhv", DCDC_HV[i]);
                        jsonObject_1.put("collvolt", samplingVs[i]);
                        jsonObject_1.put("outwarn", ERROR_state_01[i]);
                        jsonObject_1.put("inwarn", ERROR_state_02[i]);
                        jsonObject_1.put("bmswarn", ERROR_state_03[i]);
                        jsonObject_1.put("modvolt", DCDC_dianya[i]);
                        jsonObject_1.put("modele", DCDC_dianliu[i]);
                        ...
                        if (send_jsonarray.length() == DEFINE_BAR_COUNT) { // 所有数据都得上去 包括没有的仓门
                            ...
                            dataJsonObject1.put("dbm", dbm);  //柜子id
                            dataJsonObject1.put("cabtype", "6-can-48_60v混合");  //柜子id
                            dataJsonObject1.put("version", MyApplication.cab_version + "");  //柜子当前版本
                            dataJsonObject1.put("doors", send_jsonarray);
                            //第六次上传参数 电表和温湿传感器的数据上传
                            dataJsonObject1.put("cab_tem", tem_str); //温湿传感器的温度上传数据
                            dataJsonObject1.put("cab_the", hun_str); //温湿传感器的湿度上传数据
                            dataJsonObject1.put("cab_ele", ammeterTotalPower); //电表的用电量上传数据
                            ...
                            dataJsonObject1.put("ac1involt", ACDC_dianya_in[0]);
                            dataJsonObject1.put("ac2involt", ACDC_dianya_in[1]);
                            dataJsonObject1.put("ac1outvolt", ACDC_dianya_out[0]);
                            dataJsonObject1.put("ac2outvolt", ACDC_dianya_out[1]);
                            dataJsonObject1.put("ac1maxout", ACDC_gonglv[0]);
                            dataJsonObject1.put("ac2maxout", ACDC_gonglv[1]);
                            dataJsonObject1.put("ac1outele", ACDC_dianliu_in[0]);
                            dataJsonObject1.put("ac2outele", ACDC_dianliu_in[1]);
            ...
            //长链接下发  给服务器上传电柜里面的电池参数
                ...
                final String in_volt = jsonObject.getString("volt");
                final long uid10 = Long.parseLong(uid32, 32);
                syncControl(new Consumer() {
                    @Override
                    public void accept(Object o) {
                        ...
                        for (int i = 0; i < PERCENtAGES.length; i++) {
                            ...
                            int is_stop = forbiddenSp.getTargetForbidden(i);
                            if (PERCENtAGES[i] > outDoorBarPerMax && in_volt.equals(volt) && UIDS[i].equals("AAAAAAAA") && is_stop == 1) {
                                outDoorBarPerMax = PERCENtAGES[i];
                                outDoorBarIndex = i;
                            }
                        }

                        if (outDoorBarIndex == -1) {
                            showDialogInfo("没有可以租赁的电池！", "10", "1");
                            return;
                        } else {
                            showDialogInfo("请稍候！！", "5", "1");
                        }

                        final int fOutDoorBarIndex = outDoorBarIndex;
                        final String fUid32 = uid32;
                        final String fDid = did;
                        final String fUid10 = uid10 + "";
                        ...
                        if (result_out == 0) {
                            //没有写入成功
                            if (exchangeFailCount == 0) {
                                showDialogInfo("电池租赁失败，正在尝试二次租赁，请稍候！！", "10", "1");
                                SystemClock.sleep(10000);
                                onHttpReturnDataResult(orderString.toString());
                                exchangeFailCount = 1;
                            } else {
                                showDialogInfo("电池租赁失败，请拨打客服电话", "7", "1");
                                exchangeFailCount = 0;
                            }
                        } else {
                            exchangeFailCount = 0;
                            LogUtil.I("租赁成功");
                            showDialogInfo("租赁成功！！请手机尾号" + fUid10.substring(fUid10.length() - 5, fUid10.length() - 1) + "的用户拿走" + (fOutDoorBarIndex + 1) + "号仓门电池", "20", "1");
//                            HttpOutLineRentBattery httpOutLineRentBattery = new HttpOutLineRentBattery(fDid, cabinetID, BIDS[fOutDoorBarIndex], (fOutDoorBarIndex + 1) + "");
//                            httpOutLineRentBattery.start();
                            push("租赁电池", (fOutDoorBarIndex + 1) + "");
                            SystemClock.sleep(6000);
                        }
                    }
                });
            ...
            //长链接下发 回传视频信息
                ...
                httpUploadMovies.httpPost(upUrl, MoviesCreateFile.OutSideSd + "/MyCameraApp" + File.separator + day + File.separator + hour + File.separator + vname, vname, cabid, admid, token, upField, uploadPbarHandler);
                    ...
                    public void run() {
                        ...
                        if (result_out == 0) {
                            ...
                            if (fOutType.equals("2")) {
                                ...
                                showDialogInfo("电池写入成功！！请手机尾号" + fUid10.substring(fUid10.length() - 5, fUid10.length() - 1) + "的用户拿走第" + (fOutDoorBarIndex + 1) + "号仓门电池", "20", "1");
                            }
    ...
    private LinkedBlockingQueue<Consumer> replaceLinkedBlockingQueue;

    /**
     * @param consumer
     */
    private void syncControl(Consumer consumer) {
        if (consumer != null) {
