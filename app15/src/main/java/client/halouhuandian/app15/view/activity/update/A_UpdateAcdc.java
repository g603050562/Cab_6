...
public class A_UpdateAcdc extends BaseUpdateActivity {
    ...
    private void init() {
        ...
        if(door != 0){ //单个升级
            ...
            title_2.setText("正在升级第" + door + "号ACDC，请稍候！");

            UpdateInfoFormat updateInfoFormat = new UpdateInfoFormat(UpdateInfoFormat.UpdateType.updateAcdc , door ,path , type);
            UpdateController.getInstance().init(updateInfoFormat, new UpdateInfoReturnListener() {
                ...
                public void returnInfo(int door, UpdateTypeInfo type, String info) {
                    ...
                    if(type == UpdateTypeInfo.error){
                        showUpdateDialog("Acdc升级失败，程序将在10秒后返回！" , 10);
                        finishUpdate();
                    }else if(type == UpdateTypeInfo.success){
                        showUpdateDialog("Acdc升级成功，程序将在10秒后返回！" , 10);
                        finishUpdate();
                    }
                }
    ...
    private void updateIteration(int door){
        ...
        activity.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                title_2.setText("正在升级第" + mDoor + "号舱门ACDC，请稍候！");
            }
        });
        updateBar_2(mDoor,SystemConfig.getMaxAcdc());
        UpdateInfoFormat updateInfoFormat = new UpdateInfoFormat(UpdateInfoFormat.UpdateType.updateAcdc , mDoor ,path , type);
        UpdateController.getInstance().init(updateInfoFormat, new UpdateInfoReturnListener() {
            @Override
            public void returnInfo(int door, UpdateTypeInfo type, String info) {
                updateLog(info);
                if(type == UpdateTypeInfo.error){
                    showUpdateDialog(mDoor+"号acdc升级失败，程序将在10秒后返回！" , 10);
                    finishUpdate();
                }else if(type == UpdateTypeInfo.success){
                    if(mDoor < SystemConfig.getMaxAcdc()){
                        UpdateController.getInstance().onDestroy();
                        showUpdateDialog(mDoor+"号acdc升级成功" , 3);
                        updateIteration(mDoor + 1);
                    }else{
                        showUpdateDialog("全部acdc升级成功，程序将在10秒后返回！" , 10);
                        finishUpdate();
                    }
                }
            }
