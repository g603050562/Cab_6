...
public class HttpUploadMoviesPath extends Thread {
    ...
    public void run() {
        ...
        if (level.equals("3")) {
            ...
            System.out.println("movies：" + "上传路径 - " + UtilFilesDirectory.EXTERNAL_MOVIES_DIR  + File.separator + data + File.separator + hour);
            if (file.exists()) {
            ...
            System.out.println("movies：" + "上传路径 - " + UtilFilesDirectory.EXTERNAL_MOVIES_DIR );
            if (file.exists()) {
                File[] subFile = file.listFiles();
                for (int iFileLength = 0; iFileLength < subFile.length; iFileLength++) {
                    // 判断是否为文件夹
            ...
            System.out.println("movies：" + "上传路径 - " + UtilFilesDirectory.EXTERNAL_MOVIES_DIR  + File.separator + data);
            if (file.exists()) {
                File[] subFile = file.listFiles();
                for (int iFileLength = 0; iFileLength < subFile.length; iFileLength++) {
                    // 判断是否为文件夹
