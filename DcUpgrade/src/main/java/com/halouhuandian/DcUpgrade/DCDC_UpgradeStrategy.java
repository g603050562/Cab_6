 ...
 * Description:
 */
public class DCDC_UpgradeStrategy extends DC_UpgradeStrategy {

    public DCDC_UpgradeStrategy(byte address, String upgradeFilePath) {
        super(address, upgradeFilePath);
    }
    ...
    @Override
    public void execute_can(CanDeviceIoAction deviceIoAction, byte[] bytes) {
        try {
            runPduUpgrade(bytes, deviceIoAction, new Lock());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private void runPduUpgrade(final byte[] DATA, final CanDeviceIoAction deviceIoAction, final DCDC_UpgradeStrategy.Lock lock) throws Exception {
        ...
        deviceIoAction.register(connectingRequestResultId, new Consumer<byte[]>() {
            ...
            public void accept(byte[] bytes) {
                if (bytes != null) {
                    System.out.println("DCDC升级步骤4下位机返回：");
        ...
        while (true) {
            synchronized (lock) {
                ...
                while ((len = lock.transmissionLen >= 8 ? 8 : lock.transmissionLen % 8) > 0) {
                    ...
                    System.out.println("DCDC数据地址：" + lock.transmissionAddress);
        ...
        System.out.println("DCDC解除挂起：");
        CONTOUNT = CONTOUNT + 1;
        System.out.println("DCDC成功升级数量" + CONTOUNT);
        sleep(5000);
    }
