 ...
 * 离线换电 上传换电信息
 */
public class HttpOutLineDate extends Thread {

    private String number, uid, extime, in_electric, in_door, in_battery, out_battery, out_door, out_electric;
    private int dbm;
    private IFHttpOutLineDateListener ifHttpOutLineDateListener;

    /**
     *
     * @param number       电柜ID（例：04531）
     * @param uid          插入电池的UID32
     * @param extime       换电时间
     ...
     * @param dbm          当前电柜信号强度
     * @param ifHttpOutLineDateListener   接口
     */
    public HttpOutLineDate(String number, String uid, String extime, String in_battery, String in_door, String in_electric, String out_battery, String out_door, String out_electric,int dbm, IFHttpOutLineDateListener ifHttpOutLineDateListener) {
        this.number = number;
        this.uid = uid;
        this.extime = extime;
        ...
        this.dbm = dbm;
        this.ifHttpOutLineDateListener = ifHttpOutLineDateListener;
    }
    ...
    public void run() {
        ...
        if (dbm > 50 || dbm < -125) {
            System.out.println("网络：  网络：JSON：HttpOutLineDate   DBM超时");
            returnMessgae("-1", "网络：JSON：HttpOutLineDate   DBM超时", "",extime);
            return;
        }


        String path = MyApplication.apc + "Exchange/exchangev8.html";
        ...
        try {
            ...
            if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                ...
                try {
                    ...
                    System.out.println("网络：   JSON：HttpOutLineDate" + e.toString());
                    returnMessgae("-1", "JSON：HttpOutLineDate" + e.toString(), "", extime);
                }
            } else {
                System.out.println("网络：   JSON：HttpOutLineDate" +  httpResponse.getStatusLine().getStatusCode());
                returnMessgae("-1", "JSON：HttpOutLineDate" +  httpResponse.getStatusLine().getStatusCode(), "", extime);
            }
        } catch (Exception e) {
            System.out.println("网络：   JSON：HttpOutLineDate" + e.toString());
            returnMessgae("-1", "JSON：HttpOutLineDate" + e.toString(), "", extime);
    ...
    private void returnMessgae (String code, String str, String data, String extime){
        ifHttpOutLineDateListener.onHttpOutLineDateResult(code, str, data, extime);
    }
